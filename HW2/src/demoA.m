...
for index = 1:numFrames    % Draw each frame in the movie
    ...
    beta1S(index) = dbm2power(pathLossModel(ref_dist, ref_power, beta1, shadow_variance, ...
        abs(mobilePos(index)-connected_cp))); % in watts
    beta1NoS(index) = dbm2power(pathLossModel(ref_dist, ref_power, beta1, 0, ...
        abs(mobilePos(index)-connected_cp))); % in watts
    beta2S(index) = dbm2power(pathLossModel(ref_dist, ref_power, beta2, shadow_variance, ...
        abs(mobilePos(index)-connected_cp))); % in watts
    beta2NoS(index) = dbm2power(pathLossModel(ref_dist, ref_power, beta2, 0, ...
        abs(mobilePos(index)-connected_cp))); % in watts
