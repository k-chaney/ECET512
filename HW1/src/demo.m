...
mobilePos = linspace( -randi(100)-250-(randi(100)+100)*j, randi(200)-100+(randi(200)+200)*j, numFrames );
...
for index = 1:numFrames    % Draw each frame in the movie
    ...
    %    and the mobile user
    line( [real(connected_cell_position) real(mobilePos(index))], [imag(connected_cell_position) imag(mobilePos(index))] );
    axis square;
    hold off;
