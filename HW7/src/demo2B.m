...
jValue = 1;
cell_radius = 100;
numFrames = 4;    % Number of images/frames in the movie
numUsers = 1;
...
for index = 1:numFrames    % Draw each frame in the movie
    ...
    antennaLocations = [[0:1:numAntennas(index)]*dist;[0:1:numAntennas(index)]*0];
    ...
    plot( P');
    legend(user_names);
    title(['DOA Estimation-- Num antennas=' num2str(numAntennas(index))]);
    ...
    saveas(fig,['../doc/partC' num2str(numAntennas(index)) '.png']);
end
