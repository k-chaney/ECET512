...
jValue = 1;
cell_radius = 100;
numFrames = 10;    % Number of images/frames in the movie
numUsers = 1;
...
for i = 1:numUsers
    loc = randomLocation(n/2,cell_radius);
    mobilePos(:,i) = linspace( loc, ...
        loc, numFrames );
end;
...
for index = 1:numFrames    % Draw each frame in the movie
    % itterate through antenna spacings
    antennaLocations = [[0:1:numAntennas]*dist(index);[0:1:numAntennas]*0];
    ...
    plot( P');
    legend(user_names);
    title(['DOA Estimation-- Distance between antennas=lambda/' num2str(lambda/dist(index))]);
    ...
    saveas(fig,['../doc/partB' num2str(lambda/dist(index)) '.png']);
end

% outputs movie to doc folder with a framerate of 30 fps. The movie will
% last numframes/30fps
%movie2avi( movieFrames, '../doc/partB.avi','fps',5);